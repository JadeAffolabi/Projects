...
def HHblitsMSA(bin_path, db_path, input_dir, output_dir):
    ...
    for fas_file in os.listdir(input_dir):
        count += 1
        print(f"nb de fichiers traités {fas_file} : {count }:")
        if count <= 300:
            continue
        ...
        cmd = bin_path + ' -maxfilt 100000 -realign_max 100000 -d ' + db_path + ' -all -B 100000 -Z 100000 -n 3 -e 0.001 -i ' + process_file + ' -o ' + output_file + ' -oa3m ' + output_file_a3m + ' -cpu 18'
        print(cmd)
        os.system(cmd)
        # print(cmd)
        # print(count)
...
def alnFilePrepare():
    ...
    for dataset in datasets:
        ...
        HHblits_bin_path = '/home/mgevia00/miniconda3/envs/dgraphdta/bin/hhblits'  # HHblits bin path
        HHblits_db_path = '/home/mgevia00/uniclust30_2018_08/uniclust30_2018_08'  # HHblits db path
        HHfilter_bin_path = '/home/mgevia00/miniconda3/envs/dgraphdta/bin/hhfilter'  # HHfilter bin path
        reformat_bin_path = '/home/mgevia00/miniconda3/envs/dgraphdta/scripts/reformat.pl'
        convertAlignment_bin_path = '/home/mgevia00/dgraphdta/scripts/convert_alignment.py'  # convertAlignment bin path
